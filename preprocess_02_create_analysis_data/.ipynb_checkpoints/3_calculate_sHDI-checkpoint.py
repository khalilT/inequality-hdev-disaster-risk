...
gdl_data_subnational = gdl_data_subnational[["iso_code","GDLCODE",'year',"shdi"]]
...
gdl_data_subnational = gdl_data_subnational[["year","GDLCODE","shdi"]]
...
subnational_gdl_joined = subnational_gdl_joined.groupby('DisNo.').apply(lambda x: pd.Series(np.average(x["shdi"], weights=x["pop_weight"], axis=0)))
subnational_gdl_joined = subnational_gdl_joined.reset_index().rename(columns={0:"sHDI"})
